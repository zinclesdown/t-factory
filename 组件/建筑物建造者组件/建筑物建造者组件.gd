...
func _process_刷新所在子世界相关信息() -> void:
...
func _process(_delta: float) -> void:
	...
	当前鼠标图块坐标 = Vector2i(floori(get_global_mouse_position().x / 图块大小.x), floori(get_global_mouse_position().y / 图块大小.y))
...
## 切换场景也会导致被手上的蓝图实例被重设.
func 设置当前手上的蓝图实例(蓝图实例: 构筑物蓝图实例) -> void:
	if _当前手上的蓝图实例:
	...
	if 蓝图实例 and 是否位于构筑物功能有效的场景内() and 是否已设置蓝图和放置层():
